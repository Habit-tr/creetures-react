...
const SingleCategory = () => {
  ...
  useEffect(() => {
    async function fetchChallengesByCategory(name: string) {
      const { data: category } = await supabase.from('categories').select('id').match({ name }).single();
      const { data: challenges } = await supabase.from('challenges').select().eq('category_id', category?.id);
